...
func (l *etcdLock) Lock(stopChan <-chan struct{}) (<-chan struct{}, error) {
	...
	// thorough path: key exists, needs watching
	opts := &etcd.WatcherOptions{AfterIndex: etcdError.Index}
	watcher := l.client.Watcher(l.key, opts)
	watcherCh := make(chan *etcd.Response)
	...
	for {
		...
		// if it's gone now, retry immediately
		_, err := l.client.Get(ctx, l.key, nil)
		if err != nil {
			if etcdError, ok := err.(etcd.Error); ok {
				if etcdError.Code == etcd.ErrorCodeKeyNotFound {
					// try again right now
		...
		case event, ok := <-watcherCh:
			if !ok {
				return nil, errors.New("watcher closed during lock")
			}
